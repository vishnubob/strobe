...
int segments = 12;
int modeNumber = 5; // which resonant vibration mode
float phaseMax = 256.0; // maximum (based on bit depth)

// animation variables
//int currentMode = 0; // what mode are we on?
int currentAnimationStep = 0; // what frame on this mode are we on?
int timeSoFar = 0; // overall counter
int timeUntilChange = 4*256; // just arbitrarily large anyway, so use time units * 256

float t = 0.0;  // Start time
float amplitude = 50.0;  // Height of wave
float period = 500.0;  // How many pixels before the wave repeats
float dx;  // Value for incrementing X, a function of period and xspacing
float[] theta,a; // geometric phases
float[] yvalues;  // Using an array to store height values for the wave
int[] phase; // array of phases for animation
int[] previousPhase;
...
class AnimationRecord {
  ...
  AnimationRecord(int tempModeType, int tempDuration) { // each step of this array has the type and how long
    modeType = tempModeType;
    duration = tempDuration;
  }
}  

class ModeState { // each curent state has its type and time, for fading?
  long timeSoFar;
  int modeType;
...
void setup() {
  ...
  size(600,150);
  frameRate(60); // roughly the actual... this should be set accordingly to watch animations play out as they will
...
void draw() {
  background(132,112,255); // nice blue

  // calculate all the next frame info
  if(timeUntilChange > 0) {
    for(int i = 0; i< segments ; i++) { // for each strip
    ...
    if(currentAnimationStep >= MAX_ANIMATION_STEPS) {
      ...
      currentAnimationStep = 0; // this starts it over... instead, how to make it stop?
...
int calcNextFrame( ModeState myModeState, int strip) {
  ...
  case 3:
    ...
    return switchBetween(1, 83, 165, 40, 45, 165, myModeState.timeSoFar, strip); //fast was 216 (=-40)
  case 4:
    //freezeAndFan(setupTime, deltaDistance, stepDelay, velocityInMiddle, timeInMiddle, tsf, strip) {
    return freezeAndFan(200, 20, 10, 2, 250, myModeState.timeSoFar, strip); // deltaDistance was 20 originally
  case 5:
...
void renderWave(int k) {
  ...
  for (int i = 0; i < width / segments; i++) {
    ...
    // calc it
    yvalues[i] = a[i + k*width/segments] * sin((float)phase[k] / phaseMax * 2 * PI); // phase[i] is set by animation 0-1024

    // render it
    noStroke();
    fill(0);
    ellipseMode(CENTER);
    ellipse((i + k*width/segments),height/2+yvalues[i],3,3); // h/2 centers thiis
  }
}
		...
		if(tsf <= setupTime) {
			...
			if(auxModeCounter1<6) {
				if(auxModeCounter3>=stepDelay) {
					...
					if( (auxModeCounter2>=deltaDistance) ||
					     (auxModeCounter1==0 && auxModeCounter2>=(deltaDistance>>1))) {
						auxModeCounter2=0;
...
int bumpAndGrind(int setupTime, int stepTime, int velocity, int stepsPerStrip, boolean useParity, long tsf, int strip) {
	if(strip==0) {
		if(tsf<=setupTime) {
			//init
			auxModeCounter1=0;	//the strip we're currently moving
			auxModeCounter2=0;	//how much the strip should move this step
			auxModeCounter3=0;	//the counter used to see if we're at stepTime
			auxModeCounter4=0;	//when this hits stepsPerStrip, we move strips
			auxModeCounter5=1;
...
int freakOutAndComeTogether(int returnStepsPower, long tsf, int strip) {
	...
	if(tsf==0) {
//		startPosition[strip]=rand() & 255;	//check this, must be even multiple of returnSteps
                startPosition[strip] = (int)random(255); // 0 to 255
